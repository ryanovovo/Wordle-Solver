...
int diff_answer_and_guess(const int &ans_idx, const int &guess_idx){
	string answer = all_words[ans_idx];
	string guess = all_words[guess_idx];
	int diff_result = 0;
	int pow = 1;
	vector<int> is_used_answer(5, 0);
	vector<int> is_used_guess(5, 0);
	for(int i = 0; i < 5; i++){
		if(answer[i] == guess[i]){
			diff_result += pow * 2;
			is_used_answer[i] = 1;
	...
	for(int i = 0; i < 5; i++){
		pow = 1;
		if(is_used_answer[i] == 1){
			continue;
		}
		for(int j = 0; j < 5; j++){
			if(answer[i] == guess[j] && is_used_guess[j] != 1){
				diff_result += pow;
				is_used_answer[i] = 1;
				is_used_guess[j] = 1;
				break;
